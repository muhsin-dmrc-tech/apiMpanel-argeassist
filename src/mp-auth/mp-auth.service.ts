...
export class MpAuthService {
    ...
    async login(loginDto: LoginDto, ipAddress: string, deviceInfo: string) {
        ...
        const user = await this.dataSource.getRepository(MpKullanicilar).findOne({
            where: { Email: email },
            select: ['id', 'Email', 'Sifre', 'deletedAt', 'KullaniciTipi', 'YetkiliAdSoyad','FirmaAdi', 'isVerified', 'isTwoFactorEnabled'], // Şifreyi buraya ekliyoruz
        });
        ...
        if (!user.isVerified) {
            try {
                await this.dataSource.getRepository(MpLoginKayitlari).save({
                    Kullanici: user,
    ...
    private async handleEmailVerification(user: MpKullanicilar) {
        try {
    ...
    private async handleTwoFactorAuthentication(user: MpKullanicilar, ipAddress: string, deviceInfo: string) {
        try {
    ...
    private async generateAccessToken(user: MpKullanicilar, isSuccess: boolean) {
        try {
            if (isSuccess) {
                const payload = {
                    userId: user.id,
                ...
                await this.dataSource.getRepository(MpKullanicilar).update(user.id, { isActive: true });
        ...
        try {
            ...
            try {
                ...
                if (user.isActive !== guncelDurum) {
                    await this.dataSource.getRepository(MpKullanicilar).update(user.id, { isActive: guncelDurum });
                }
    ...
    async twoFactorLogin(code: string, email: string) {
        try {
            ...
            try {
                ...
                await this.dataSource.getRepository(MpKullanicilar).update(user.id, {
                    isActive: true,
                });
                return {
                    success: true,
    ...
    async twoFactorCodeGenerate(user: MpKullanicilar) {
        ...
        if (twoFactorType === 'sms') {
            const message = `Merhaba ${user.YetkiliAdSoyad}, iki faktörlü giriş için doğrulama kodunuz: ${twoFactorCode}`;
            try {
    ...
    async register(registerDto: RegisterDto, ipAddress: string, deviceInfo: string) {
        ...
        try {
            ...
            const savedUser = await this.dataSource.transaction(async (manager) => {
                // Kullanıcı kaydı
                const user = await manager.getRepository(MpKullanicilar).save({
                    Email: email,
                    YetkiliAdSoyad: yetkiliFullName,
                    FirmaAdi: firmaAdi,
                    KullaniciTipi: 1,
                ...
                // Login kaydı
                await manager.getRepository(MpLoginKayitlari).save({
                    KullaniciId: user.id,
                    GirisZamani: new Date(),
                    IPAdresi: ipAddress,
                    CihazBilgisi: deviceInfo,
    ...
    async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
        ...
        // Reset URL oluştur
        const resetUrl = `${process.env.MP_FRONTEND_URL}/auth/reset-password/change?token=${resetToken}&email=${email}`;
        try {
            await this.mailService.sendEmailWithTemplate(
    ...
    async sendVerificationEmail(user: MpKullanicilar) {
        if (user.isVerified) {
            return false;
        }
        ...
        const verificationUrl = `${process.env.MP_FRONTEND_URL}/auth/verify-email?token=${verificationToken}`;
    ...
    async verifyEmailToken(
        ...
        try {
            ...
            await this.dataSource.getRepository(MpKullanicilar).save(user);
    ...
    async getUserByEmail(email: string) {
        return await this.dataSource.getRepository(MpKullanicilar).findOne({
            where: { Email: email },
        });
    }
    ...
    async updateEmail(userId: number, email: string) {
        ...
        try {
            const emailvisible = await this.dataSource.getRepository(MpKullanicilar).findOne({
                where: { Email: email },
            });
            ...
            await this.dataSource.getRepository(MpKullanicilar).update(userId, { Email: email, verifiedAt: null, isVerified: false, isActive: false });
            try {
                const updateuser = await this.dataSource.getRepository(MpKullanicilar).findOne({
                    where: { id: userId }
                });
    ...
    async updateUserInfo(fullName: string,firmaAdi: string, phoneNumber: string, userId: number) {
        try {
            ...
            try {
                await this.dataSource.getRepository(MpKullanicilar).update(userId, { YetkiliAdSoyad: fullName,FirmaAdi:firmaAdi, Telefon: phoneNumber });

            } catch (error) {
                throw new Error(error)
            }
            const updatedUser = await this.dataSource.getRepository(MpKullanicilar).findOne({
                where: { id: userId },
            });
    ...
    async updateUserAvatar(filePath: string, userId: number) {
        try {
            ...
            try {
                await this.dataSource.getRepository(MpKullanicilar).update(userId, { ProfilResmi: profilimage });
    ...
    async accountDelete(userId: number) {
        try {
            ...
            try {
                await this.dataSource.getRepository(MpKullanicilar).softDelete({ id: userId });
    ...
    async avatarDelete(userId: number) {
        try {
            ...
            try {
                await this.dataSource.getRepository(MpKullanicilar).update(userId, { ProfilResmi: null });
            } catch (error) {
