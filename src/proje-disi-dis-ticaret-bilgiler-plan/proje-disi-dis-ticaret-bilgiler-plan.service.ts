...
@Injectable()
export class ProjeDisiDisTicaretBilgilerPlanService {
    constructor(
        @InjectRepository(ProjeDisiDisTicaretBilgilerPlan)
        private readonly PlanRepository: Repository<ProjeDisiDisTicaretBilgilerPlan>,
    ...
    /* async getActivePlanlar() {
        ...
        const plan = await this.dataSource
            .getRepository(ProjeDisiDisTicaretBilgilerPlan)
    ...
    async getPlanlar(userId: number, query: any) {
        ...
        const sort = query.sort || 'ProjeDisiDisTicaretBilgilerPlanID';
        ...
        const queryBuilder = this.dataSource.getRepository(ProjeDisiDisTicaretBilgilerPlan).createQueryBuilder('plan')
        ...
        const allowedSortFields = ['Donem', 'Firma', 'Durum', 'ProjeDisiDisTicaretBilgilerPlanID'];
    ...
    async create(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number }) {
        ...
        try {
            ...
            return await this.PlanRepository.createQueryBuilder('plan')
                .leftJoinAndSelect('plan.Firma', 'Firma')
                .leftJoinAndSelect('plan.Donem', 'Donem')
                .where('plan.ProjeDisiDisTicaretBilgilerPlanID = :ProjeDisiDisTicaretBilgilerPlanID', { ProjeDisiDisTicaretBilgilerPlanID: plan.ProjeDisiDisTicaretBilgilerPlanID })
                .getOne();
        } catch (error) {
    ...
    async update(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeDisiDisTicaretBilgilerPlanID: number }) {
        ...
        if (!data.ProjeDisiDisTicaretBilgilerPlanID) {
            throw new BadRequestException(`ProjeDisiDisTicaretBilgilerPlanID gereklidir`);
        }
        if (!data.FirmaID || !data.DonemID || !data.ProjeDisiDisTicaretBilgilerPlanID) {
            throw new BadRequestException(`Firma ID, Donem ID ve ProjeDisiDisTicaretBilgilerPlan ID zorunludur`);
        }

        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeDisiDisTicaretBilgilerPlanID: data.ProjeDisiDisTicaretBilgilerPlanID } });
    ...
    async delete(userId: number, data: any) {
        ...
        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeDisiDisTicaretBilgilerPlanID: data.itemId } });
            if (plan) {
