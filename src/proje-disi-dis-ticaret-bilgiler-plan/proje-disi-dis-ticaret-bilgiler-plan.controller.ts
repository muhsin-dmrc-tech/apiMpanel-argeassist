...
@Controller('proje-disi-dis-ticaret-bilgiler-plan')
export class ProjeDisiDisTicaretBilgilerPlanController {
    constructor(
        private readonly planService: ProjeDisiDisTicaretBilgilerPlanService,
    ) { }
    ...
    @Roles(2)
    @Post('update')
    async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeDisiDisTicaretBilgilerPlanID: number }, @Request() req) {
        if (!req.user.userId) {
            throw new BadRequestException('Kullanıcı Kimliği gereklidir');
        }
