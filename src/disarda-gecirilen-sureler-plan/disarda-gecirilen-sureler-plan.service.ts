...
@Injectable()
export class DisardaGecirilenSurelerPlanService {
    constructor(
        @InjectRepository(DisardaGecirilenSurelerPlan)
        private readonly PlanRepository: Repository<DisardaGecirilenSurelerPlan>,
    ...
    /* async getActivePlanlar() {
        ...
        const plan = await this.dataSource
            .getRepository(DisardaGecirilenSurelerPlan)
    ...
    async getPlanlar(userId: number, query: any) {
        ...
        const sort = query.sort || 'DisardaGecirilenSurelerPlanID';
        ...
        const queryBuilder = this.dataSource.getRepository(DisardaGecirilenSurelerPlan).createQueryBuilder('plan')
        ...
        const allowedSortFields = ['Donem', 'Firma', 'Durum', 'DisardaGecirilenSurelerPlanID'];
    ...
    async create(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number }) {
        ...
        try {
            ...
            return await this.PlanRepository.createQueryBuilder('plan')
                .leftJoinAndSelect('plan.Firma', 'Firma')
                .leftJoinAndSelect('plan.Donem', 'Donem')
                .where('plan.DisardaGecirilenSurelerPlanID = :DisardaGecirilenSurelerPlanID', { DisardaGecirilenSurelerPlanID: plan.DisardaGecirilenSurelerPlanID })
                .getOne();
        } catch (error) {
    ...
    async update(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, DisardaGecirilenSurelerPlanID: number }) {
        ...
        if (!data.DisardaGecirilenSurelerPlanID) {
            throw new BadRequestException(`DisardaGecirilenSurelerPlanID gereklidir`);
        }
        if (!data.FirmaID || !data.DonemID || !data.DisardaGecirilenSurelerPlanID) {
            throw new BadRequestException(`Firma ID, Donem ID ve DisardaGecirilenSurelerPlan ID zorunludur`);
        }

        try {
            const plan = await this.PlanRepository.findOne({ where: { DisardaGecirilenSurelerPlanID: data.DisardaGecirilenSurelerPlanID } });
    ...
    async delete(userId: number, data: any) {
        ...
        try {
            const plan = await this.PlanRepository.findOne({ where: { DisardaGecirilenSurelerPlanID: data.itemId } });
            if (plan) {
