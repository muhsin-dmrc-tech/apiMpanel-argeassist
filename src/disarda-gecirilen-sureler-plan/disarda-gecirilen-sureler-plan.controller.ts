...
@Controller('disarda-gecirilen-sureler-plan')
export class DisardaGecirilenSurelerPlanController {
    constructor(
        private readonly planService: DisardaGecirilenSurelerPlanService,
    ) { }
    ...
    @Roles(2)
    @Post('update')
    async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, DisardaGecirilenSurelerPlanID: number }, @Request() req) {
        if (!req.user.userId) {
            throw new BadRequestException('Kullanıcı Kimliği gereklidir');
        }
