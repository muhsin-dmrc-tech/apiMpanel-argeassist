...
@Controller('proje-ilerleme-bilgiler-plan')
export class ProjeIlerlemeBilgilerPlanController {
    constructor(
        private readonly planService: ProjeIlerlemeBilgilerPlanService,
    ) { }
    ...
    @Roles(2)
    @Post('update')
    async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeIlerlemeBilgilerPlanID: number, ProjeID: number }, @Request() req) {
        if (!req.user.userId) {
            throw new BadRequestException('Kullanıcı Kimliği gereklidir');
        }
