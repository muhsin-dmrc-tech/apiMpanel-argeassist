...
@Injectable()
export class ProjeIlerlemeBilgilerPlanService {
    constructor(
        @InjectRepository(ProjeIlerlemeBilgilerPlan)
        private readonly PlanRepository: Repository<ProjeIlerlemeBilgilerPlan>,
    ...
    /* async getActivePlanlar() {
        ...
        const plan = await this.dataSource
            .getRepository(ProjeIlerlemeBilgilerPlan)
    ...
    async getPlanlar(userId: number, query: any) {
        ...
        const sort = query.sort || 'ProjeIlerlemeBilgilerPlanID';
        ...
        const queryBuilder = this.dataSource.getRepository(ProjeIlerlemeBilgilerPlan).createQueryBuilder('plan')
        ...
        const allowedSortFields = ['Donem', 'Firma','Proje', 'Durum', 'ProjeIlerlemeBilgilerPlanID'];
    ...
    async create(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeID: number }) {
        ...
        try {
            ...
            return await this.PlanRepository.createQueryBuilder('plan')
                ...
                .leftJoinAndSelect('plan.Donem', 'Donem')
                .leftJoinAndSelect('plan.Proje', 'Proje')
                .where('plan.ProjeIlerlemeBilgilerPlanID = :ProjeIlerlemeBilgilerPlanID', { ProjeIlerlemeBilgilerPlanID: plan.ProjeIlerlemeBilgilerPlanID })
                .getOne();
        } catch (error) {
    ...
    async update(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeIlerlemeBilgilerPlanID: number, ProjeID: number }) {
        ...
        if (!data.ProjeIlerlemeBilgilerPlanID) {
            throw new BadRequestException(`ProjeIlerlemeBilgilerPlanID gereklidir`);
        }
        if (!data.FirmaID || !data.DonemID || !data.ProjeID || !data.ProjeIlerlemeBilgilerPlanID) {
            throw new BadRequestException(`Firma ID, Proje ID, Donem ID ve ProjeIlerlemeBilgilerPlan ID zorunludur`);
        }

        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeIlerlemeBilgilerPlanID: data.ProjeIlerlemeBilgilerPlanID } });
    ...
    async delete(userId: number, data: any) {
        ...
        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeIlerlemeBilgilerPlanID: data.itemId } });
            if (plan) {
