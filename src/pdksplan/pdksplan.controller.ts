...
@Controller('pdks-plan')
export class PdksplanController {
    constructor(
        private readonly planService: PdksplanService,
    ) { }
   ...
   /*  @UseGuards(JwtAuthGuard)
    ...
    async getActiveFirmalar(
        ...
        return this.planService.getActiveFirmalar();
    }
    ...
    @Roles(2)
    @Post('update')
    async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, PDKSPlanID: number }, @Request() req) {
        if (!req.user.userId) {
            throw new BadRequestException('Kullanıcı Kimliği gereklidir');
        }
