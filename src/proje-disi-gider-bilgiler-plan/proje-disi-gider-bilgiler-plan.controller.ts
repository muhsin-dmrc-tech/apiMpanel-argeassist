...
@Controller('proje-disi-gider-bilgiler-plan')
export class ProjeDisiGiderBilgilerPlanController {
    constructor(
            private readonly planService: ProjeDisiGiderBilgilerPlanService,
        ) { }
        ...
        @Roles(2)
        @Post('update')
        async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeDisiGiderBilgilerPlanID: number }, @Request() req) {
            if (!req.user.userId) {
                throw new BadRequestException('Kullanıcı Kimliği gereklidir');
            }
