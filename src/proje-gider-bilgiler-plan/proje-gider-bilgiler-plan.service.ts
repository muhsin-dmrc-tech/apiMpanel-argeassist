...
@Injectable()
export class ProjeGiderBilgilerPlanService {
    constructor(
        @InjectRepository(ProjeGiderBilgilerPlan)
        private readonly PlanRepository: Repository<ProjeGiderBilgilerPlan>,
    ...
    /* async getActivePlanlar() {
        ...
        const plan = await this.dataSource
            .getRepository(ProjeGiderBilgilerPlan)
    ...
    async getPlanlar(userId: number, query: any) {
        ...
        const sort = query.sort || 'ProjeGiderBilgilerPlanID';
        ...
        const queryBuilder = this.dataSource.getRepository(ProjeGiderBilgilerPlan).createQueryBuilder('plan')
        ...
        const allowedSortFields = ['Donem', 'Firma', 'Proje', 'Durum', 'ProjeGiderBilgilerPlanID'];
    ...
    async create(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeID: number }) {
        ...
        try {
            ...
            return await this.PlanRepository.createQueryBuilder('plan')
                ...
                .leftJoinAndSelect('plan.Donem', 'Donem')
                .leftJoinAndSelect('plan.Proje', 'Proje')
                .where('plan.ProjeGiderBilgilerPlanID = :ProjeGiderBilgilerPlanID', { ProjeGiderBilgilerPlanID: plan.ProjeGiderBilgilerPlanID })
                .getOne();
        } catch (error) {
    ...
    async update(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeGiderBilgilerPlanID: number, ProjeID: number }) {
        ...
        if (!data.ProjeGiderBilgilerPlanID) {
            throw new BadRequestException(`ProjeGiderBilgilerPlanID gereklidir`);
        }
        if (!data.FirmaID || !data.DonemID || !data.ProjeID || !data.ProjeGiderBilgilerPlanID) {
            throw new BadRequestException(`Firma ID, Proje ID, Donem ID ve ProjeGiderBilgilerPlan ID zorunludur`);
        }

        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeGiderBilgilerPlanID: data.ProjeGiderBilgilerPlanID } });
    ...
    async delete(userId: number, data: any) {
        ...
        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeGiderBilgilerPlanID: data.itemId } });
            if (plan) {
