...
@Injectable()
export class ProjeGelirBilgilerPlanService {
    constructor(
        @InjectRepository(ProjeGelirBilgilerPlan)
        private readonly PlanRepository: Repository<ProjeGelirBilgilerPlan>,
    ...
    /* async getActivePlanlar() {
        ...
        const plan = await this.dataSource
            .getRepository(ProjeGelirBilgilerPlan)
    ...
    async getPlanlar(userId: number, query: any) {
        ...
        const sort = query.sort || 'ProjeGelirBilgilerPlanID';
        ...
        const queryBuilder = this.dataSource.getRepository(ProjeGelirBilgilerPlan).createQueryBuilder('plan')
        ...
        const allowedSortFields = ['Donem', 'Firma', 'Proje', 'Durum', 'ProjeGelirBilgilerPlanID'];
    ...
    async create(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeID: number }) {
        ...
        try {
            ...
            return await this.PlanRepository.createQueryBuilder('plan')
                ...
                .leftJoinAndSelect('plan.Donem', 'Donem')
                .leftJoinAndSelect('plan.Proje', 'Proje')
                .where('plan.ProjeGelirBilgilerPlanID = :ProjeGelirBilgilerPlanID', { ProjeGelirBilgilerPlanID: plan.ProjeGelirBilgilerPlanID })
                .getOne();
        } catch (error) {
    ...
    async update(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeGelirBilgilerPlanID: number, ProjeID: number }) {
        ...
        if (!data.ProjeGelirBilgilerPlanID) {
            throw new BadRequestException(`ProjeGelirBilgilerPlanID gereklidir`);
        }
        if (!data.FirmaID || !data.DonemID || !data.ProjeID || !data.ProjeGelirBilgilerPlanID) {
            throw new BadRequestException(`Firma ID, Proje ID, Donem ID ve ProjeGelirBilgilerPlan ID zorunludur`);
        }

        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeGelirBilgilerPlanID: data.ProjeGelirBilgilerPlanID } });
    ...
    async delete(userId: number, data: any) {
        ...
        try {
            const plan = await this.PlanRepository.findOne({ where: { ProjeGelirBilgilerPlanID: data.itemId } });
            if (plan) {
