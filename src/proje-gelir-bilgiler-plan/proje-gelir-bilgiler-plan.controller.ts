...
@Controller('proje-gelir-bilgiler-plan')
export class ProjeGelirBilgilerPlanController {
    constructor(
        private readonly planService: ProjeGelirBilgilerPlanService,
    ) { }
    ...
    @Roles(2)
    @Post('update')
    async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeGelirBilgilerPlanID: number, ProjeID: number }, @Request() req) {
        if (!req.user.userId) {
            throw new BadRequestException('Kullanıcı Kimliği gereklidir');
        }
