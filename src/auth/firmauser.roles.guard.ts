...
export class YetkiRolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (user.userTypeEnum === 1) {
      queryBuilder.leftJoinAndMapOne('personel.Firma', Firma, 'Firma', 'Firma.FirmaID = personel.IliskiID AND Firma.IsDeleted != 1');
    } else if (user.userTypeEnum === 3) {
      queryBuilder.leftJoinAndMapOne('personel.Teknokent', Teknokentler, 'Teknokent', 'Teknokent.TeknokentID = personel.IliskiID AND Teknokent.IsDeleted != 1');
    }
