...
@Injectable()
export class FirmaMuafiyetBilgilerPlanService {
    constructor(
        @InjectRepository(FirmaMuafiyetBilgilerPlan)
        private readonly PlanRepository: Repository<FirmaMuafiyetBilgilerPlan>,
    ...
    /* async getActivePlanlar() {
        ...
        const plan = await this.dataSource
            .getRepository(FirmaMuafiyetBilgilerPlan)
    ...
    async getPlanlar(userId: number, query: any) {
        ...
        const sort = query.sort || 'FirmaMuafiyetBilgilerPlanID';
        ...
        const queryBuilder = this.dataSource.getRepository(FirmaMuafiyetBilgilerPlan).createQueryBuilder('plan')
        ...
        const allowedSortFields = ['Donem', 'Firma', 'Durum', 'FirmaMuafiyetBilgilerPlanID'];
    ...
    async create(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number }) {
        ...
        try {
            ...
            return await this.PlanRepository.createQueryBuilder('plan')
                .leftJoinAndSelect('plan.Firma', 'Firma')
                .leftJoinAndSelect('plan.Donem', 'Donem')
                .where('plan.FirmaMuafiyetBilgilerPlanID = :FirmaMuafiyetBilgilerPlanID', { FirmaMuafiyetBilgilerPlanID: plan.FirmaMuafiyetBilgilerPlanID })
                .getOne();
        } catch (error) {
    ...
    async update(userId: number, data: { Durum: boolean, FirmaID: number, DonemID: number, FirmaMuafiyetBilgilerPlanID: number }) {
        ...
        if (!data.FirmaMuafiyetBilgilerPlanID) {
            throw new BadRequestException(`FirmaMuafiyetBilgilerPlanID gereklidir`);
        }
        if (!data.FirmaID || !data.DonemID || !data.FirmaMuafiyetBilgilerPlanID) {
            throw new BadRequestException(`Firma ID, Donem ID ve FirmaMuafiyetBilgilerPlan ID zorunludur`);
        }

        try {
            const plan = await this.PlanRepository.findOne({ where: { FirmaMuafiyetBilgilerPlanID: data.FirmaMuafiyetBilgilerPlanID } });
    ...
    async delete(userId: number, data: any) {
        ...
        try {
            const plan = await this.PlanRepository.findOne({ where: { FirmaMuafiyetBilgilerPlanID: data.itemId } });
            if (plan) {
