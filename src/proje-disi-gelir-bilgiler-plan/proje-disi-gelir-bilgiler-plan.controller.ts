...
@Controller('proje-disi-gelir-bilgiler-plan')
export class ProjeDisiGelirBilgilerPlanController {
    constructor(
        private readonly planService: ProjeDisiGelirBilgilerPlanService,
    ) { }
    ...
    @Roles(2)
    @Post('update')
    async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeDisiGelirBilgilerPlanID: number }, @Request() req) {
        if (!req.user.userId) {
            throw new BadRequestException('Kullanıcı Kimliği gereklidir');
        }
