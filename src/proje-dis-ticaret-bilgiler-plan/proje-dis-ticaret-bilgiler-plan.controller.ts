...
@Controller('proje-dis-ticaret-bilgiler-plan')
export class ProjeDisTicaretBilgilerPlanController {
    constructor(
            private readonly planService: ProjeDisTicaretBilgilerPlanService,
        ) { }
        ...
        @Roles(2)
        @Post('update')
        async update(@Body() data: { Durum: boolean, FirmaID: number, DonemID: number, ProjeDisTicaretBilgilerPlanID: number, ProjeID: number }, @Request() req) {
            if (!req.user.userId) {
                throw new BadRequestException('Kullanıcı Kimliği gereklidir');
            }
